...
class SettingsBehaviour(unittest.TestCase):
    ...
    def test_window_is_lazy_reusable_and_has_independent_working_checkboxes(self):
        ...
        assert(#checkboxes==0)
        SlashCmdList.QOL()
        assert(#checkboxes==12 and #UISpecialFrames==1)
        ...
        SlashCmdList.QOL(); SlashCmdList.QOL(); assert(#checkboxes==12)
        ''')

    def test_saved_false_values_loaded_after_lua_are_preserved(self):
        self.lua.execute('''
        BetaQoLDB={autoAccept=false, autoTurnIn=false, fastLoot=false, enterConfirm=false, rangeColor=false, whisperDoubleClick=false, backspaceDestroy=false, squareMinimap=false, questNameplateBag=false, chatArrowKeys=false, questLogXP=false, questDropRate=false}
    ...
    def test_saved_settings_round_trip_across_fresh_lua_runtime(self):
        self.lua.execute("SlashCmdList.QOL(); clickSetting(1,false); clickSetting(2,false); clickSetting(4,false); clickSetting(6,false); clickSetting(7,false); clickSetting(8,true); clickSetting(9,false); clickSetting(10,false); clickSetting(11,false); clickSetting(12,false)")
        values = {key: self.lua.globals().BetaQoLDB[key]
                  for key in ('autoAccept', 'autoTurnIn', 'fastLoot', 'enterConfirm', 'rangeColor', 'whisperDoubleClick', 'backspaceDestroy', 'squareMinimap', 'questNameplateBag', 'chatArrowKeys', 'questLogXP', 'questDropRate')}
        ...
        fresh.execute("emit('ADDON_LOADED','BetaQoL'); SlashCmdList.QOL(); "
                      ...
                      "assert(not checkboxes[4]:GetChecked() and checkboxes[5]:GetChecked() and not checkboxes[6]:GetChecked() and not checkboxes[7]:GetChecked() and checkboxes[8]:GetChecked() and not checkboxes[9]:GetChecked() and not checkboxes[10]:GetChecked() and not checkboxes[11]:GetChecked() and not checkboxes[12]:GetChecked())")
    ...
    def test_missing_or_invalid_preferences_get_defaults_without_overwriting_false(self):
        ...
        assert(BetaQoLDB.squareMinimap==true and checkboxes[8]:GetChecked())
        assert(BetaQoLDB.questNameplateBag==true and checkboxes[9]:GetChecked())
        ''')
    ...
    def test_minimap_and_quest_icon_choices_survive_repeated_runtime_restarts(self):
        self.lua.execute("emit('ADDON_LOADED','BetaQoL'); SlashCmdList.QOL(); "
                         "assert(checkboxes[8]:GetChecked() and checkboxes[9]:GetChecked())")
        current = self.lua
        for minimap, quest_icon in ((True, True), (False, True), (True, False), (False, False)):
            with self.subTest(minimap=minimap, quest_icon=quest_icon):
                current.execute(f"clickSetting(8,{str(minimap).lower()}); "
                                f"clickSetting(9,{str(quest_icon).lower()})")
                ...
                for _ in range(2):
                    ...
                    self.assertEqual(fresh.globals().checkboxes[8].checked, minimap)
                    self.assertEqual(fresh.globals().checkboxes[9].checked, quest_icon)
                    current = fresh
